...
## v1.3.18 (2026-05-10) — 구버전 updater 잔류 CMD 정리 보강

### 수정
- 새 updater 시작 시 pre-1.3.18 외부 적용 스크립트가 남긴 `DBJARVIS_apply_update_*` CMD와 `findstr /R /C:"PID"` 프로세스를 정리하도록 보강
- 외부 helper 실행을 숨김 실행으로 통일해 보조 콘솔 창 노출 가능성을 축소
- 기준 버전을 1.3.18로 상향해 1.3.16/1.3.17 설치본 모두 새 updater를 받도록 갱신

## v1.3.17 (2026-05-10) — 업데이트 후 findstr CMD 창 잔류 수정

### 수정
- 업데이트 외부 적용 스크립트의 updater 종료 대기 로직에서 `tasklist | findstr` 파이프를 제거
- `Wait-Process` 기반 대기로 변경해 `findstr /R /C:"PID"` CMD 창이 남는 문제를 방지
- 외부 적용 스크립트와 updater 자기 교체 스크립트 실행 시 숨김 실행 옵션을 강화
- 기준 버전을 1.3.17로 상향해 1.3.16 설치본에서도 새 updater를 받을 수 있도록 배포 기준 갱신

## v1.3.16 (2026-05-10) — 1.3.15 설치본 업데이트 감지용 릴리스

### 버전/배포
- v1.3.15 설치본과 최신 GitHub Release가 같은 버전이면 업데이트 팝업이 뜨지 않는 정상 동작을 확인
- v1.3.15 설치본에서 새 업데이트가 감지되도록 기준 버전을 1.3.16으로 상향
- `source/VERSION`과 빌드 기본 버전을 1.3.16으로 동기화
- `AutoMain.exe` 파일/제품 버전을 1.3.16.0으로 갱신
- 배포 ZIP/SHA256 및 설치판 재생성 기준을 1.3.16으로 갱신

## v1.3.15 (2026-05-09) — 업데이트 다운로드 권한 문제 수정

### 수정
- 업데이트 다운로드/압축해제 임시 폴더를 설치 폴더 부모가 아닌 사용자 AppData 작업 폴더로 변경
- 외부 적용 스크립트와 백업 폴더도 AppData 작업 폴더를 사용하도록 변경
- 설치 폴더 부모 권한이 없어도 파일 교체가 가능하도록 디렉터리 rename 대신 `robocopy` 기반 적용/롤백으로 변경
- 다운로드 임시 폴더 생성 실패 원인을 로그에 남기도록 보강
- `AutoMain.exe` 파일/제품 버전을 1.3.15.0으로 갱신

## v1.3.14 (2026-05-09) — 설치/업데이트 예외 처리 보강

### 수정
- 설치판에서 예외 발생 시 PyInstaller의 `Unhandled exception in script` 창이 뜨지 않도록 로그와 한글 오류 안내로 처리
- 업데이터 임시 폴더 삭제 실패를 재시도 후 경고로만 남기도록 변경
- `UPDATE_CONFIG.json` 손상 시 기본 설정으로 복구해 업데이트 확인이 중단되지 않도록 보강
- `AutoMain.exe` 파일/제품 버전을 1.3.14.0으로 갱신
